 ...
 */
final class NativePageView implements RenderNode
{
    private readonly RenderNode $content;

    /**
     * @param array<int, RenderNode> $pages
     */
    public function __construct(
        ...
        $page = $pages[$clamped] ?? new RenderSizedBox(0, 0);

        $dots = array_map(
            static fn (int $i): RenderNode => new RenderPadding(
                EdgeInsets::only(left: $i > 0 ? Tokens::SPACE_XS : 0),
                new RenderContainer(width: 8, height: 8, radius: 4, background: $i === $clamped ? Tokens::ink() : Tokens::border()),
            ),
            range(0, $count - 1),
        );

        $nav = RenderFlex::row([
            new NativeIconCircle('chevron_left', 32.0, action: "toggle:{$fieldName}", meta: ['next' => (string) max(0, $clamped - 1)]),
            new Flexible(new RenderCenter(RenderFlex::row($dots))),
            new NativeIconCircle('chevron_right', 32.0, action: "toggle:{$fieldName}", meta: ['next' => (string) min($count - 1, $clamped + 1)]),
        ], crossAxisAlignment: CrossAxisAlignment::CENTER);

        $this->content = RenderFlex::column([
            new RenderContainer($page, height: $height),
            new RenderPadding(EdgeInsets::only(top: Tokens::SPACE_SM), $nav),
