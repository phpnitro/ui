...
final class SwitchToggle extends Widget
{
    ...
    public static function make(string $name, string $label, bool $on = false, ?Color $activeColor = null): self
    {
        return new self($name, $label, $on, $activeColor);
    }

    public function render(): string
    {
        $color = $this->activeColor ?? Theme::primary();
        $activeClass = "peer-checked:{$color->backgroundClass()}";
        ...
        return sprintf(
            ...
            . '<span class="w-10 h-6 rounded-full bg-gray-300 dark:bg-gray-600 %s '
            ...
            $this->on ? ' checked' : '',
            htmlspecialchars($activeClass, ENT_QUOTES),
