...
final class NativeRichTextTest extends TestCase
{
    ...
    private function paintToCommands(RenderRichText $node, float $maxWidth = 1000.0): array
    {
    ...
    {
        $commands = $this->paintToCommands(new RenderRichText([
    ...
    {
        $one = new RenderRichText([new TextSpan('court')]);
        $twoLines = new RenderRichText([new TextSpan('un texte assez long pour forcer deux lignes ici')]);
